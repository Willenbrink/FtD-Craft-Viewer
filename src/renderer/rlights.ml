...
let update_light_values shader light =
  ...
  let color = Vector4.(create
                         ((Color.r light.color |> float) /. 255.0)
                         ((Color.g light.color |> float) /. 255.0)
                         ((Color.b light.color |> float) /. 255.0)
                         ((Color.a light.color |> float) /. 255.0)
                      )
  in
...
let create_light typ position target color shader =
  ...
  let light = {
    enabled = true; typ; position; target; color;
